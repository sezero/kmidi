...
static void drawProg(int,int,int,int,Boolean),drawPan(int,int,Boolean),
  ...
  deleteTextCB(Widget w,XtPointer data,XtPointer call_data),
#endif
...
#else
#define XAW_BITMAP_DIR DEFAULT_PATH "/bitmaps"
#endif

String bitmapdir = XAW_BITMAP_DIR;
Boolean arrangetitle,savelist;
static char **current_flist = NULL;
static int c_voices = 0, last_voice = 0, voices_num_width;
...
#else
      snprintf(local_buf,sizeof(local_buf),p,"0.2i");
#endif
...
#else
        Panel->channel[ch].chorusdepth = n;
#endif
      ...
      for(i=0; i<MAX_XAW_MIDI_CHANNELS; i++)
        if (Panel->v_flags[i]) {
          if (Panel->v_flags[i] == FLAG_NOTE_OFF) {
            Panel->ctotal[i] -= (int16)( DELTA_VEL * delta_time) ;
            if (Panel->ctotal[i] <= 0) {
              Panel->ctotal[i] = 0;
              Panel->v_flags[i] = 0;
            }
...
static void drawVoices(void) {
  ...
  sprintf(local_buf, "%3d/%d", last_voice, c_voices);
...
#else
        drawChorus(i,Panel->channel[i].chorusdepth);
#endif
      } else {
...
#else
        psmenu = (Widget *)realloc((char *)psmenu, sizeof(Widget)*(submenu_n + 2));
#endif
...
#else
  snprintf(local_buf,sizeof(local_buf),"TiMidity 0.2i");
#endif
  if(ctl->trace_playing)
...
#else
  snprintf(cbuf,sizeof(cbuf),"TiMidity 02.i");
#endif
